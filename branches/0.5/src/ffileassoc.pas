...
begin
  if sFileName <> EmptyStr then
    begin
      bmpTemp:= PixMapManager.LoadBitmapEnhanced(sFileName, 32, sbtnIcon.Color);
      if Assigned(bmpTemp) then
        begin
  ...
  begin
    ...
    if sFileName <> EmptyStr then
      ...
      lbFileTypes.Items.Objects[Index]:= PixMapManager.LoadBitmapEnhanced(sFileName, gIconsSize, Color);
    lbFileTypes.Repaint;
